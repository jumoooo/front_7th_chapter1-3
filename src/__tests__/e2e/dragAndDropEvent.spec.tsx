...
test.describe('드래그 앤 드롭 일정 이동 E2E 테스트', () => {
  ...
  test('달력의 일정을 드래그 후 일정이 겹치는 곳 위로 이동하면 겹침 알람 팝업이 발생해야 한다.', async ({
    ...
    // 1. A 일정 생성: 오늘 날짜에 시간 겹침이 발생할 일정 생성
    await saveSchedule(page, {
      title: 'A 회의',
      date: today, // 오늘 날짜
      startTime: '09:00',
    ...
    // 2. B 일정 생성: 내일 날짜에 일정 생성 (나중에 오늘로 드래그할 일정)
    await saveSchedule(page, {
      title: 'B 회의',
      date: tomorrow, // 내일 날짜
      startTime: '14:00',
    ...
    // 월별 뷰에서 드래그할 일정 Box 찾기 (B 일정)
    const monthView = page.getByTestId('month-view');

    // B 일정이 달력에 표시될 때까지 대기
    const eventBox = await monthView.locator('text=B 회의').first();
    await expect(eventBox).toBeVisible();
    ...
    // 3. B 일정을 A 일정이 있는 날짜(오늘)로 드래그 앤 드롭
    // 드롭할 대상 날짜 셀 찾기 (A 일정이 있는 셀 - 오늘 날짜)
    const tableBody = monthView.locator('tbody');
    const cells = tableBody.locator('td');
    ...
    // A 일정이 있는 셀 찾기 (오늘 날짜이고 A 일정이 있는 셀)
    let targetCell;
    for (let i = 0; i < (await cells.count()); i++) {
      const cell = cells.nth(i);
      const dayText = await cell.locator(`text=${todayDay}`).first();
      const hasEventBox = await cell.locator('text=A 회의').count();

      // 오늘 날짜이고 A 일정이 있는 셀을 찾음
      if ((await dayText.count()) > 0 && hasEventBox > 0) {
        targetCell = cell;
        break;
      }
    ...
    // 드래그 앤 드롭 수행 (B 일정을 오늘 날짜로 이동)
    await draggableBox.dragTo(targetCell!, { force: true });

    // 4. 겹침 경고 팝업이 나타나는지 확인
    // B 일정(14:00-15:00)을 오늘로 이동하면 A 일정(09:00-15:00)과 시간이 겹치므로 알람이 나타나야 함
  ...
  test('드래그 앤 드롭으로 반복 일정을 이동하면 확인 다이얼로그가 나타나고 확인 후 반복제거, 날짜가 변경된다', async ({
    ...
    // 이벤트 리스트에서도 확인
    const eventList = page.getByTestId('event-list');
    await expect(eventList.getByText('반복 회의').first()).toBeVisible();

    // 반복 아이콘이 없어야 함 (반복이 취소되었으므로)
    // '반복 회의' 일정 항목을 찾고, 그 항목 내에서 반복 아이콘이 없는지 확인
    const repeatedMeetingText = eventList.getByText('반복 회의').first();
    // 텍스트 요소의 가장 가까운 부모 요소(일정 항목)를 찾음
    const repeatedMeetingItem = repeatedMeetingText
      .locator(
        'xpath=ancestor::li | ancestor::div[contains(@class, "event")] | ancestor::div[contains(@class, "item")]'
      )
      .first();
    // 해당 일정 항목 내에서 반복 아이콘을 찾아서 없어야 함
    const repeatIconInItem = repeatedMeetingItem.locator('[data-testid="RepeatIcon"]');
    await expect(repeatIconInItem).not.toBeVisible();
  });
});
